# 參考https://ww2.mathworks.cn/matlabcentral/fileexchange/52966-artificial-bee-colony-abc-in-matlab
import numpy as np
import matplotlib
...
for generation in range(0 , N_GENERATION):
    ...
    for scout_bee in range(0 , SCOUT_BEE_NUM):
        ...
        # 依照原來蜂群演算法的概念，如果迭代到最後已經找到最多的花蜜量(找到global maximum or global minimum)
        # 那意味著new_position採集到的花蜜量不再大於原本的花蜜量，所以會讓bad_position_freq不斷累積直到等於LIMIT
        # 最後考察蜂會再去重新找一個新的position，導致失去最好的position
        # 因此在這裡用decision決定到底要"大於等於"還是"大於"
        decision = np.random.randint(0 , 2 , size = 1)[0]
        if decision == 0:
            # 若在new_position採集到的花蜜量new_honey_amount大於等於原本花蜜量honey_amount[scout_bee]
            ...
            if new_honey_amount > honey_amount[scout_bee]:
                position[scout_bee , :] = new_position
                honey_amount[scout_bee] = new_honey_amount
            # 若在new_position採集到的花蜜量new_honey_amount小於等於原本花蜜量honey_amount[scout_bee]
            # 則bad_position_freq[scout_bee]加1
            else:
                bad_position_freq[scout_bee] += 1
    ...
    for greedy_bee in range(0 , GREEDY_BEE_NUM):
        ...
        # 最後考察蜂會再去重新找一個新的position，導致失去最好的position
        # 因此在這裡用decision決定到底要"大於等於"還是"大於"
        decision = np.random.randint(0 , 2 , size = 1)
        if decision == 0:
    ...
    honey_amount_max_index = honey_amount.argmax()
    honey_amount_min_index = honey_amount.argmin()
    position_honey_amount_max = position[honey_amount_max_index , :]
    position_honey_amount_min = position[honey_amount_min_index , :]
    # 3. 考察蜂
    for inspect_bee in range(0 , INSPECT_BEE_NEM):
        ...
        if bad_position_freq[inspect_bee] >= LIMIT:
            Φ = np.random.uniform(-1 , 1 , [1 , DIM]) # 隨機生成範圍在[-1 , 1]間的係數Φ
            new_position = position_honey_amount_min + Φ * (position_honey_amount_max - position_honey_amount_min)
            new_position = np.clip(new_position , position_bound[0] , position_bound[1])
