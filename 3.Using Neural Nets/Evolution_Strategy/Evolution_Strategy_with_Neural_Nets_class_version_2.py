...
class accessory(object):    
    ...
    def get_reward_mp(self , sigma , shapes , params , env , ep_max_step , continuous_a):
...
class ESNN(object):
    def __init__(self , n_kid , lr , sigma , n_core , config , momentum , accessory):
        self.n_kid = n_kid 
        self.lr = lr
        self.sigma = sigma
        self.n_core = n_core
    ...
    def train(self):
        ...
        for k_id in range(0 , self.n_kid):
            ...
            jobs.append(self.pool.apply_async(self.accessory.get_reward_mp , (self.sigma , 
                                                                              self.shapes , 
                                                                              self.net_params , 
                                                                              self.env , 
...
if __name__ == "__main__":  
    ...
    for g in range(0 , N_GENERATION):
        t0 = time.time()
        esnn.train()
        [net_r , _] = ACCESSORY.get_reward_mp(esnn.sigma , esnn.shapes , esnn.net_params , esnn.env , esnn.config['ep_max_step'] , esnn.config['continuous_a'])
