...
N_SCOUT = 20                 # half of the training population
...
def get_reward(shapes , params , env , ep_max_step , continuous_a , train = True):
    if train:
        noise = np.random.randn(params.shape[0])
        params = params + SIGMA * noise
...
def train(net_shapes , net_params , move , weight , pool):
    ...
    jobs = [pool.apply_async(get_reward , (net_shapes ,
                                           ...
                                           CONFIG['ep_max_step'] ,
                                           CONFIG['continuous_a']))\
            for _ in range(0 , N_SCOUT)]

    rewards , noises = [] , []
    for job in jobs:
        temp = job.get()
        rewards.append(temp[0])
        noises.append(temp[1])
    ...
    for i , reward_index in enumerate(rewards_index):
        ...
        cumulative_update = cumulative_update + weight[i] * noises[reward_index , :]
...
if __name__ == "__main__":
    ...
    rank = np.arange(1 , N_SCOUT + 1)
    weight = np.maximum(0 , np.log(N_SCOUT / 2 + 1) - np.log(rank))
    weight = weight / weight.sum() - 1 / N_SCOUT
    ...
    for generation in range(0 , N_GENERATION):
        ...
        testing_reward = get_reward(net_shapes , net_params , env , CONFIG['ep_max_step'] , CONFIG['continuous_a'] , train = False)
